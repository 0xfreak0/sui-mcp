...
export function serverInstructions(): string {
  ...
  return [
    ...
    "",
    "For an investigation: identify_address first; summarize_address_flows for what an address took in and sent out; trace_funds to follow value (read stop_reason and the unfollowed branches before the path), trace_flow_graph for every branch and find_flow_path between two addresses; find_funding_source or find_funding_sources to attribute; get_address_fanout to measure a funder before trusting shared funding; build_wallet_edges to cluster; screen_address and classify_deposit_address for exposure and exchange deposits; analyze_attack_tx and summarize_incident_losses for exploits; get_upgrade_history for who could upgrade a package at a given time; resolve_bridge_transfer for cross-chain exits. Record conclusions with save_finding and render them with export_case.",
    "",
